export default {
    accounts: {
      ...
      requestBodyAdmission: 'Tiered Request Body Admission',
      requestBodyAdmissionHint: 'Isolate large Codex requests so they cannot occupy all shared account slots.',
      ...
      requestBodyAdmissionCapacityHint: 'Heavy requests use at most 20% of account concurrency, with no more queued requests than heavy slots. Each user may run one large request and queue one more; the recovery channel, including remote compaction, has one global slot and one global waiter.',
      requestBodyAdmissionInvalid: 'Request body limits must be positive, satisfy normal < heavy < recovery, and not exceed 64 MB.',
      expiresAt: 'Expires At',

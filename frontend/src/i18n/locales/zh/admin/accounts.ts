export default {
    accounts: {
      ...
      requestBodyAdmission: '请求体分级准入',
      requestBodyAdmissionHint: '隔离 Codex 大请求，避免占满共享账号槽位。',
      requestBodyNormalLimit: '普通通道上限（MB）',
      requestBodyHeavyLimit: '重请求通道上限（MB）',
      requestBodyRecoveryLimit: '恢复通道上限（MB）',
      requestBodyAdmissionCapacityHint: '重请求最多使用此账号 20% 的并发，账号等待数不超过重请求槽位数；每个用户最多执行 1 个大请求并排队 1 个；恢复通道（含远程压缩）全局仅 1 个槽位和 1 个等待请求。',
      requestBodyAdmissionInvalid: '请求体大小必须为正数，满足普通通道 < 重请求通道 < 恢复通道，且不能超过 64 MB。',
      expiresAt: '过期时间',

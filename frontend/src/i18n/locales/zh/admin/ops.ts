export default {
    ops: {
      ...
      failedToLoadUserErrorDistribution: '加载按用户错误分布失败',
      failedToLoadInvestigation: '加载调查结论失败',
      failedToLoadPerformanceDiagnostics: '加载性能诊断失败',
      ...
      userLatency: {
        ...
        topAvg: '平均耗时最高',
        deleted: '已删除',
        summary: '{count} 个成功请求，平均 {avg}',
        emptyUser: '该用户在当前窗口内没有成功请求耗时数据。',
        bucketDetails: '请求时长 {range}'
      },
      userErrors: {
        title: '按用户请求错误分布',
        summary: '{users} 位用户，共 {errors} 个 SLA 错误',
        unknownUser: '未知用户',
        unknownType: '未知错误',
        userTotal: '该用户合计：{count}',
        topLimit: '仅展示错误数最高的前 {count} 位用户。'
      },
      switchRate: '平均账号切换',
      investigation: {
        title: '调查结论',
      ...
      performance: {
        title: '慢请求原因诊断',
        primaryCause: '当前首要慢因：{cause}，占慢请求的 {share}',
        noSlowRequests: '所选时段暂无命中阶段阈值的慢请求或失败请求',
        slowRate: '慢请求率',
        endToEndTtftP95: '端到端 TTFT P95',
        ingestionLoss: '诊断遥测自本次服务启动后已丢弃 {dropped} 条、写入失败 {failed} 条，所选时段的慢因统计可能不完整。',
        ingestionBacklog: '诊断遥测写入队列接近满载（{depth}/{capacity}），慢因数据可能延迟显示。',
        causeRanking: '慢因排名',
        causeTrend: '慢因趋势',
        slowRequestsAxis: '慢请求数',
        impactScope: '影响范围',
        queueP95: '排队 P95',
        requests: '请求数',
        mainCause: '主要原因',
        detailedAnalysis: '详细分析与错误钻取',
        dimensions: { user: '用户', account: '账号', model: '模型' },
        causes: {
          healthy: '正常',
          user_queue: '用户并发排队',
          heavy_queue: '重请求通道排队',
          recovery_queue: '恢复通道排队',
          request_body_lane_queue: '请求通道排队',
          account_queue: '账号并发排队',
          body_read: '请求体上传与读取',
          routing: '内部路由与调度',
          upstream_ttft: '上游首 Token 慢',
          stream_gap: '流式输出中途卡顿',
          failover: '账号切换与重试',
          upstream: '上游处理慢',
          internal: '平台内部处理',
          request_too_large: '请求体过大',
          queue_rejected: '并发队列已满',
          recovery_deadline: '恢复请求超过截止时间',
          client_canceled: '客户端取消请求',
          no_available_account: '无可用账号',
          upstream_error: '上游请求失败',
          request_rejected: '请求在网关被拒绝'
        }
      },
      concurrencyTrend: {
        title: '所选时段用户并发峰值',
        subtitle: '按普通、重请求和恢复通道统计并发峰值与请求响应时间',
        systemDemand: '系统总需求',
        systemActive: '系统占用',
        systemWaiting: '系统排队',
        currentDemand: '当前需求',
        currentActive: '占用',
        currentWaiting: '排队',
        demandStatistics: '并发需求（请求数）',
        responseTimeStatistics: '响应时间（ms）',
        selectUser: '选择用户曲线',
        removeUser: '移除用户曲线',
        axis: '并发请求数',
        unlimited: '不限',
        ...
        laneEmpty: '该通道在所选时段暂无并发活动',
        empty: '所选时段暂无并发活动',
        partialCoverage: '并发快照仅保留 24 小时，当前仅显示 {start} 至 {end} 的可用数据。',
        unavailableCoverage: '所选时段已超出并发快照的 24 小时保留范围，无法提供历史并发数据。',
      ...
      tooltips: {
        ...
        userErrors: '按用户和错误类型统计 SLA 错误，排除业务限制；用户按错误总数从高到低排列。',
